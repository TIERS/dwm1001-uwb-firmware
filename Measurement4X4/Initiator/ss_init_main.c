...
*			This code is the implementation of an autocalibration algorithm for decawave devices. It is based on the code example explained above provided by decawave.
*			This code is designed to be flashed into the first initiator of the network which will listen to the UART channel. 
* 
* @attention
*
...
#include "C:\Users\qing\Downloads\dwm1001-examples-master_4_4\examples\shared_var.h"
/*Device ID*/
int my_id = 1; //CHANGE THIS TO THE DESIRED ID BEFORE PROGRAMMING THE DEVICE (1-DEVICE STARTING AS INITIATOR, 2,3,4 responders

/*NUMBER OF MEASUREMENTS PER RESPONDER*/
#define  MAX_ITER 2 
...
/*Messages with byte 9 = 'S' are named standard and are sent only to the target responder until the number of measurements defined by MAX_ITER are recieved*/
/*Messages with byte 9 = 'F' are broadcast when all the measurements from one responder have been received so the whole network can store that information*/
...
/*uint8 measurements_matrix[4][4][4] = {
...
uint8 measurements_matrix[4][4][4] = {0};
...
{
  /*Set poll message's source as my id*/
  tx_poll_msg[SOURCE_IDX] = my_id;
  tx_poll_msg2[SOURCE_IDX] = my_id;

  /* Loop forever initiating ranging exchanges. */

  /*while the number of measurements received from a responder is less than the defined number of iterations, keep ranging*/
  /*Set the target responder in counter-clockwise order*/
  if(iter_cnt<MAX_ITER)
  {
    ...
    /*Set the corresponding responder as destination of the poll messages*/
		printf("%d -> %d \r\n", my_id, resp_id);
  ...
  {
  	/*Once the desired number of mesaurements has been received, increase counter to change the target responder in the following loop*/
    last++;
    /*Calculate mean and standard deviation for that responder*/
    mean = total/MAX_ITER;
    ...
    /*Broadcast mean, standard deviation and responder id so every device in the network can store that information*/
    ...
    measurements_matrix[my_id - 1][resp_id - 1][M1_IDX]= mean_msb;
    measurements_matrix[my_id - 1][resp_id - 1][M1_IDX + 1]= mean_lsb;
    measurements_matrix[my_id - 1][resp_id - 1][S1_IDX]= SD_msb;
    measurements_matrix[my_id - 1][resp_id - 1][S1_IDX + 1]= SD_lsb;

    iter_cnt = 0;
    /*If all the responders have been polled by the current initiator rise the change flag so the following node becomes initiator instead*/
    if(last == 3)
    {
      last = 0;
      change = true;
  ...
  * set by dwt_setrxaftertxdelay() has elapsed. */
  ...
  {
		// printf("I: Stuck here"); 
	// It work here, anyway, it will keep sending when others are die, and become alive when 
	// others connected into the group, and all others are initialed as responder. so if this 
  // node is initaltor, no problem.
	};
  ...
  {		
    ...
    * As the sequence number field of the frame is not relevant, it is cleared to simplify the validation of the frame. */
    ...
    /* Check if the received message comes from the current target responder and it is intended for an initiator (to avoid possible interferences) */
    ...
    {	
      ...
      /*Store distance, add it to the total to calculate the mean and increase iteration counter*/
...
int ss_resp_run(void)
{
	...
	uint32_t dw_crnt_time = 0; // Current Timestamp when it change
	uint32_t dw_start_time = 0; // Last Timestamp when it change
	uint32_t dw_time_diff = 0; // Last Timestamp when it change
	
  dwt_readsystime((uint8 *) &dw_time);
  dw_start_time = (uint32_t)(dw_time>>8);
  ...
  while (!((status_reg = dwt_read32bitreg(SYS_STATUS_ID)) & (SYS_STATUS_RXFCG | SYS_STATUS_ALL_RX_TO | SYS_STATUS_ALL_RX_ERR)))
  {
		...
		dwt_readsystime((uint8 *) &dw_time);
		dw_crnt_time  = (uint32_t)(dw_time>>8);
		
		/*if(dw_crnt_time - dw_start_time > 80000000){
			printf("R	: Stuck here %u \r\n", (dw_crnt_time - dw_start_time));
			change = true;
			return 1;
		}*/
		// printf("R \r\n");
  ...
  if (status_reg & SYS_STATUS_RXFCG)
  {
    ...
    /* Check that the frame is a poll sent to this responder in particular*/
    if((rx_buffer2[DEST_IDX-1]=='R') && ( rx_buffer2[DEST_IDX] == my_id))
    {
     /* Check if it is a standard message */
      if(rx_buffer2[SOURCE_IDX+1] == 'S')
      {
        ...
        tx_resp_msg[DEST_IDX] = rx_buffer2[SOURCE_IDX]; //Reply to the device that sent the poll message
    ...
    /*Check if the message is a finish message and store the broadcasted information*/
    if((rx_buffer2[SOURCE_IDX+1] == 'F')&&(rx_buffer2[DEST_IDX-1]=='R'))
    {
      exter_resp_id = rx_buffer2[DEST_IDX];
      exter_init_id = rx_buffer2[SOURCE_IDX];
      measurements_matrix[exter_init_id-1][exter_resp_id-1][M1_IDX] = rx_buffer2[MEAN_IDX];
      measurements_matrix[exter_init_id-1][exter_resp_id-1][M1_IDX+1] = rx_buffer2[MEAN_IDX+1];
      measurements_matrix[exter_init_id-1][exter_resp_id-1][S1_IDX] = rx_buffer2[STD_IDX];
      measurements_matrix[exter_init_id-1][exter_resp_id-1][S1_IDX+1] = rx_buffer2[STD_IDX+1];

      /*Check if the finish message was intended for me and if it is comming from the preceding responder in the network to raise the change flag and print the information */
      printf("%d -> %d \r\n", rx_buffer2[SOURCE_IDX], my_id);
			if(rx_buffer2[DEST_IDX] == my_id)
      {
        if(((rx_buffer2[SOURCE_IDX] == 1)&&(my_id == 2))||((rx_buffer2[SOURCE_IDX] == 2)&&(my_id == 3))||((rx_buffer2[SOURCE_IDX] == 3)&&(my_id == 4))||((rx_buffer2[SOURCE_IDX] == 4)&&(my_id == 1)))
        {
					printf("Sending msg: \r\n");
	        ...
	        //printf("{ ");
		        for(int j = 0; j < 4; j++)
		        {
		          ...
		          for(int k = 0; k < 4; k++)
		          {
		            printf("%d , ", measurements_matrix[i][j][k]);
		          }
		          //printf("} ");
		        }
		        printf("\r\n ");
