...
int cycle_cnt = 0;
int init_cnt;
/*Device initial state flag*/
bool my_flag = true; //true -- starts as a initiator, false -- starts as a responder
/*Array with received UART command and defined start command for comparison(aux)*/
char c[2] ;
char aux[2] = {'S','\0'} ;
...
int main(void)
{
  ...
  * As this example only handles one incoming frame with always the same delay and timeout, those values can be set here once for all. */
  ...
  if(my_flag)
    dwt_setrxtimeout(65000); // Maximum value timeout with DW1000 is 65ms for initiator 
  else
    dwt_setrxtimeout(0); // Maximum value timeout with DW1000 is 0ms  for responders
  //-------------dw1000  ini------end---------------------------	
  // IF WE GET HERE THEN THE LEDS WILL BLINK
    ...
    // Loop forever responding to ranging requests. 
	  ...
	  uint32_t dw_last_time = 0; // Last Timestamp when it change
    ...
    while (1)
    {
      ...
      // boUART_getc(c); 
			c[0] = aux[0];
      if(c[0] == aux[0]) //check if start command received
      { 
        if(my_flag) //check whether the device is configured as initiator at the moment
        {
				//printf("I   \r\n");
				dwt_setrxtimeout(6500);
          ss_init_run();
        }
        ...
        dw_time_diff =  dw_crnt_time - dw_last_time;
				if(	dw_time_diff > 82000000){
					change = 1;
					//printf("Change start timeStamp idff %u \r\n",dw_time_diff);
				}
        /*If all the responders have been asked change configuration of the required devices so the next one becomes initiator*/
        if(change) 
        {
					dw_last_time = dw_crnt_time;
					//printf("Change start timeStamp %u idff %u \r\n", dw_crnt_time, dw_time_diff);
          cycle_cnt ++ ;
          if(cycle_cnt < 2) //first cycle hasn't been completed
          {
            change = false;
            resp_cnt = 0;
            init_cnt = 0;
            my_flag = !my_flag;
            if(my_flag){
							printf("responder becomes initiator\r\n");
              dwt_setrxtimeout(6500); // Maximum value timeout with DW1000 is 65ms 							
						}
            else{
              dwt_setrxtimeout(0); // Maximum value timeout with DW1000 is 65ms
							//printf("responder becomes responder\r\n");	
						}
          } //iteration if
          
          /*first initiator becomes initiator again and resets all variables to go to the initial state*/ 
          else 
          {
              c[0] = '0';
              app_uart_flush();
              //printf("Waiting for Start order\r\n");
              change = false;
              my_flag = true;
              cycle_cnt = 0;
              resp_cnt = 0;
