...
int iteration_cnt = 0;
bool my_flag = false; //true -- starts as a initiator, false -- starts as a responder
//char c[2] ;
//char aux[2] = {'S','\0'} ;
...
int main(void)
{
  ...
  /*Initialization UART*/
  boUART_Init ();
  printf("Singled Sided Two Way Ranging RESP 2\r\n");
  ...
  * As this example only handles one incoming frame with always the same delay and timeout, those values can be set here once for all. */
  ...
  if(my_flag)
    dwt_setrxtimeout(65000); // Maximum value timeout with DW1000 is 65ms for initiators
  else
    ...
    while (1)
    {
        ...
        else
        {
          ss_resp_run(); 
       	...
       	/*If all the responders have been asked by the current initiator, change configuration of the required devices so it becomes responder and
       	 the next one becomes initiator*/
        if(change)
        {
            change = false;
            resp_cnt = 0;
            ...
            if(my_flag)
              dwt_setrxtimeout(6500); // Set Maximum value timeout with DW1000 to 65ms when it becomes initiator
            else
              dwt_setrxtimeout(0); // Set Maximum value timeout with DW1000 to 0ms when it becomes responder
          } //change if
      } //while
